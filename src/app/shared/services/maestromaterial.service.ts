...
export class MaestroMaterialService {
  ...
  private mainUrlRest: string = 'https://qas.yapu.pe/iperc-api/api/';
  ...
  constructor(private userService: UserService) {
    ...
    this.listaMaestroMaterial = sp.web.lists.getByTitle(
      Variables.lists.MaestroMaterial
    );
  }
  ...
  async getSolicitudMateriales(
    ...
    let query = this.listaMaestroMaterial.items
      .expand(...expandFields)
      .select(...selectFields);
    // debugger;
    let filterArr = [];
    if (!administrator)
      filterArr.push(
        `( (${Variables.columns.CreadoPor}/Id  eq ${usuario.Id}) or (${Variables.columns.ResponsableLaboratorio}/Id  eq ${usuario.Id})  or  (${Variables.columns.EjecutivoComercial}/Id  eq ${usuario.Id}) or  (${Variables.columns.ResponsableDT}/Id  eq ${usuario.Id}) or (${Variables.columns.ResponsableRDM}/Id  eq ${usuario.Id}))`
      );

    if (filter) {
      if (filter.Title && filter.Title.trim()) {
        filterArr.push(
          `(substringof('${filter.Title}',${Variables.columns.TextoBreveMaterial}) or substringof('${filter.Title}',${Variables.columns.TextoBreveMaterial}) or substringof('${filter.Title}',${Variables.columns.TextoBreveMaterial}))`
        );
      }
      ...
      if (filter.Etapa && filter.Etapa.length) {
        filterArr.push(
          `(${filter.Etapa.map(
            (x) => `(${Variables.columns.MaestroFLujoEtapa}/Id eq '${x}')`
          ).join(' or ')})`
        );
      }
