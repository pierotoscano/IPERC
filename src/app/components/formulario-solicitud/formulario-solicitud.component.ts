...
export class FormularioSolicitudComponent extends FormularioAT implements OnInit {
  ...
  addControl(campo): void {
    ...
    if (campoEstructura[0] !== undefined) {
      ...
      this.form.get(campo.Title).markAsTouched();
            ...
            if (campo.Title === Variables.columns.IdCanalDistribucion) {
              if (this.form.controls[campo.Title].value.length > 0) {
                this.form.controls[campo.Title].value.forEach(item => {
                  const itemMD = this.datosMaestros.maestroDesplegables.find(desplegable => desplegable.Id === item);
                  itemMD && (detail += itemMD.Title + ', ');
                });
              ...
              if (this.form.controls[campo.Title].value > 0) { // Valida si tiene valor el desplegable
                const itemMD = this.datosMaestros.maestroDesplegables.find(desplegable => desplegable.Id === this.form.controls[campo.Title].value);
                itemMD && (detail = itemMD.Title);
              } else {
                detail = '';
              }
  ...
  format(format: number, controlName: string, validMajorZero = false): void {
    // console.log('formar');
    // console.log(this.form.controls[controlName].value);
    let valid = true;

    if (validMajorZero && parseInt(this.form.controls[controlName].value) === 0) {
      valid = false;
    }

    valid && (this.form.controls[controlName].setValue(Funciones.format(format, this.form.controls[controlName].value)));
  }
